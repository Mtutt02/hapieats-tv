...
export default async function TVPage() {
  try {
    ...
    STATION_DIAL.forEach((slug, i) => {
      ...
      channels.push({
        number: i + 1, name: st.name, icon: st.icon ?? '', description: st.description ?? '',
        category: st.theme ?? 'Community', currentTitle: playlist[0]?.title ?? 'Off Air',
      })
    })
    let liveNum = 90
    for (const ls of liveStreams ?? []) {
      ...
      channels.push({
        number: liveNum++, name: (ls.channel as { name: string } | null)?.name ?? 'Live Channel',
        icon: '', description: 'Live right now', category: 'LIVE', currentTitle: ls.title,
        muxPlaybackId: ls.mux_playback_id, isLive: true,
      })
    }
    return (<AppShell fullWidth><TVBrowser channels={channels} /></AppShell>)
  } catch {
    return (<AppShell fullWidth><TVBrowser channels={[]} /></AppShell>)
  }
}
